...
class FullFetchHandler(V4Handler):
    ...
    def __init__(self, repo_root: Path, event_store=None, depth_limit: int = 2):
        ...
        self._queue: list[tuple[str, int, str]] = []  # (qid, depth, reason)
        self._done: dict[str, str] = {}               # qid → fetched_at timestamp
        self._rules: list[dict] = []  # full_fetch_rule_registered payloads
        self._load_snapshot()

    def _load_snapshot(self) -> None:
        """Populate _done and _queue from full_fetch_state.csv written by previous runs."""
        import csv
        ...
        with ffs.open(newline="", encoding="utf-8") as fh:
            for row in csv.DictReader(fh):
                ...
                if status == "complete":
                    self._done[qid] = str(row.get("fetched_at", "") or "")
                elif status == "pending":
                    ...
                    if qid not in self._done and not any(q == qid for q, _, _ in self._queue):
                        self._queue.append((qid, depth, reason))
    ...
    def _on_event(self, event: dict) -> None:
        ...
        if etype == "seed_registered":
            ...
            if qid and qid not in self._done:
                self._enqueue(qid, depth=0, reason="seed")
            ...
            qid = canonical_qid(str(payload.get("qid", "") or ""))
            if qid and qid not in self._done:
                self._enqueue(qid, depth=1, reason="entity_marked_relevant")

        elif etype == "entity_fetched":
            qid = canonical_qid(str(payload.get("qid", "") or ""))
            if qid:
                self._done[qid] = str(event.get("timestamp_utc", "") or "")
                self._queue = [(q, d, r) for q, d, r in self._queue if q != qid]
            ...
            if obj_depth > self._depth_limit:
                return
            if obj in self._done or any(q == obj for q, _, _ in self._queue):
                return
            if self._passes_rules(subject, pid, obj):
                self._enqueue(obj, depth=obj_depth, reason="full_fetch_rule")

    def _enqueue(self, qid: str, depth: int, reason: str = "") -> None:
        if qid not in self._done and not any(q == qid for q, _, _ in self._queue):
            self._queue.append((qid, depth, reason))
    ...
    def do_next(self, *, languages: list[str] | None = None) -> int:
        ...
        if not self._queue:
            return 0
        qid, depth, _reason = self._queue.pop(0)
    ...
    def _write(self, proj_dir: Path) -> None:
        rows = [[qid, "complete", "", "", fetched_at] for qid, fetched_at in sorted(self._done.items())]
        rows += [[qid, "pending", depth, reason, ""] for qid, depth, reason in self._queue]
        self._atomic_write_csv_rows(
            proj_dir / "full_fetch_state.csv",
            ["qid", "status", "depth", "reason", "fetched_at"],
            rows,
        )
