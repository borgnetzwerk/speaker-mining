...
def build_targets_from_phase2_lookup(root: str | Path) -> tuple[list[dict[str, str]], pd.Series, pd.DataFrame]:
    ...
    if programs_path.exists():
        ...
        if "name" in programs_df.columns:
                ...
                target_rows.append(
                    {
                        "mention_id": _stable_program_mention_id(
                            "broadcasting_program", "broadcasting_programs.name", program_name
                        ),
                        "mention_type": "broadcasting_program",
                        "mention_label": program_name,
                        "context": "source=data/20_candidate_generation/broadcasting_programs.csv; source_column=name",
                    }
                )
