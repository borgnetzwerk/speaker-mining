...
## Open issues
* source_coverage should use and show different data quality tiers.
* Do not reproduce color palettes - load it from the central node
* Always keep the property PID AND (shortened) property label in file name

* broadcasting_program comparisons:
  * The Sex or gender comparison currently does not add up to 100 %. That must be fixed.
  * The order of the Shows does not stay the same. It should always be the same order: Always descending by number of episodes we have of that particular show.
  * All cross-show Comparisons should be stacked bar charts.
  * There should also be sub-group-comparisons:
    * All german talk shows
    * All german podcasts
    * All german
    * All english
  * Property coverage comparison:
    * The current property coverage comparison is wrong. It wrongfully merges two different concepts and must be split into two:
      * AVERAGE values per property (decimal number from 0 to any positive number), showing the average amount of values that an apperances may have.
      * Property coverage (percentage number from 0 % to 100 %), showing the binary precence of a property in an appereance. It does not matter how many values that guest has - just "has property: true" is enough to be counted once, or 0 if otherwise. 
    * Each of these Property coverage comparison concepts should also exist twice: Once by guest, once by appereance
  * The property table should also contain a short statistic on the data availability (e.g. how many entries had wikidata (Tier 1/2) vs. how many total (Tier 1-4)
* Treemaps are currently not generated for apperances, just for guests. Always generate both.

## Fundamental principle: Visualization that is generated by guest should also have a by appereance version
Ideally, these are combined into one visualization. This combined visualization should try to minimize information duplication: If a label is written once, try to group both versions there: Either side by side (two subplots side by side, with only the left one having labels, but both having the same order so the labels from left also apply for the right), or within the same plot, e.g. as grouped bar chart. 


## Definition Page-Rank Visualization
Step 1: Createa a node for every instance (not classes).
  * these include for example epsiodes, guests, moderators, broadcasting_programs
  * the only exclusion criterium: "is a class"

Step 2: Link every node that is connected via a claim.
* Count the number of links pointing towards this node from any other node. Increase the size according to this number

The node with the most inlinks should be greatest.

---

## Color palette
We seem to have lost otabe oko in the treemap phase. And: We don't seem to be using one central color palette.

Using the following regex: `#[\dA-Z]+`, we see that 10 files in `.\speakermining\src\process\analysis` hardcode some form of color palettes. Expected behaviour would be: One place, such as color_registry, hardcodes the colors once, and everyone else uses these. In almost every case, it will be enough to retrieve them from the node once they are assigned. There are some exceptions, such as tier colors, or colors for `unknown/no data`, `unclassified`, or `other`. Still: Every color that is defined should be in one single source of truth, and everyone else imports from there (or ideally, from the QID directly after it has been asssigned a color).
...
## General issues
* broadcasting_programs should be defined over show_groups.csv. If we want to add them there, we can add them to a "members" column there. This way, a show may be member of multiple groups. The show_type column must be removed from `broadcasting_programs.csv`

## Layout, particular for Appearances and unique
* Any kind of bar chart: Unique and "other" should not share the same bar space as the values - the highest bar should get 100 % of the width. If we allow "others" or "unknown" into the same space, they will have much higher values and take up 80  % of the space. We must keep them separate, likely just as a textual label or similar. For   
* Legends should always have the highest count on top (some are currently sorted in reverse order)


## Fundamental design flaw: Too much space used not for bars.
Currently, the bar space is around 10 to 20 % of our width. This is bad.

We must focus on the bars. The need to be clearly visible. We must ensure everything else is positioned to allow them to take the space they need. Labels need to be left, but e.g. legends can be just below the subtitle or on the bottom, from left to right. Legends may also take two rows if that is easier to read. 


### Cross: Stacked bar charts
* Two subplots, Appearances left, unique guests right.
  * Order remains the same (sorted by total appearances)
  * Only the left one has labels (since the order is identicals, labels don't need to be duplicated)
  * Labels may have line breaks (only a maximum amount of characters per line, try to break at meaningful places, e.g. spaces)
    * This applies to Axis labels AND legend labels
  
* Cross-show comparisons are currently not "Appearances and unique guests in one visualization"
* Average values per appearance is duplicated:
  * "Property Coverage by Show" and "Binary coverage by appearance" are identical visualizations. Generally, the four new visualizations are good, but some improvements:
    * The old version "Property Coverage by Show" had a better layout, short and simple title, with details in the subtitle.
    * The linear color gradient does not work for average value per guest\appearance. Use a logarithmic scale here.

### Tree Map
Almost perfect - just switch left and right (Appearances always first, then unique)


## math is wrong for average value per guest show comparisons
We currently say the average StarTalk guest has 81.36 occupations and 57.55 awards recieved. I looked at the guests, and as far as I can see, no guest has more than 25 occupations (which is Richard Dawkins, Q44461)

canonical_entity_id,canonical_label,wikidata_id,appearance_count,rank,show_id
ce_83fe208169e0,Eugene Mirman,Q320185,31,1,startalk
ce_f21d31da9dcf,Richard Dawkins,Q44461,30,2,startalk
ce_0d6a3753761b,Leighann Lord,Q19666008,1,3,startalk
ce_33d4944d1cd1,George Takei,Q110154,1,4,startalk
ce_327c9b9d83e8,James Cameron,Q42574,1,5,startalk
ce_73a6e7de4314,Charles Liu,Q59673671,1,6,startalk
ce_7083ebeb6a48,Norman Lear,,1,7,startalk
ce_76597a8b4745,Janna Levin,Q6155075,1,8,startalk
ce_9a61ab7222a2,Christopher Nolan,Q25191,1,9,startalk
ce_c5e4090bee90,Saul Austerlitz,Q113844987,1,10,startalk
ce_dca2a263e0e4,Chuck Nice,Q5115638,1,11,startalk
