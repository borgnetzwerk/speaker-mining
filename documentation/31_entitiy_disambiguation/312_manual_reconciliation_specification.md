...
2. Sort/filter by deterministic_alignment_status and requires_human_review.
3. Cluster/normalize key text fields (for example names, role labels, organization labels).
4. Decide row-level outcome:
	- aligned_confirmed
	- aligned_corrected
	- unresolved
	- conflict
5. Fill reviewer metadata fields before export.
6. Export reviewed file as CSV with stable headers.
